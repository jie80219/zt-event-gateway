...
auto_auth {
  method "approle" {
    config = {
      role_id_file_path                   = "/vault/creds/worker/role_id"
      secret_id_file_path                 = "/vault/creds/worker/secret_id"
...
template {
  ...
  contents    = <<-EOT
  {{- with pkiCert "pki/issue/worker" "common_name=worker.zt.local" "uri_sans=spiffe://zt.local/worker" "ttl=24h" -}}
  {{ .Cert }}{{ .CA }}
