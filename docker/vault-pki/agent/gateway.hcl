...
auto_auth {
  method "approle" {
    config = {
      role_id_file_path                   = "/vault/creds/gateway/role_id"
      secret_id_file_path                 = "/vault/creds/gateway/secret_id"
...
template {
  ...
  contents    = <<-EOT
  {{- with pkiCert "pki/issue/gateway" "common_name=gateway.zt.local" "uri_sans=spiffe://zt.local/gateway" "ttl=24h" -}}
  {{ .Cert }}{{ .CA }}
