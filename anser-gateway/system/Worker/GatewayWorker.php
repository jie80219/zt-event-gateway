...
class GatewayWorker extends WorkerRegistrar
{
    ...
    public function initWorker(): Worker
    {
        ...
        $webWorker->onWorkerStart = static function (Worker $worker) use ($config) {
            ...
            $serviceConfigPath = \PROJECT_CONFIG . 'Service.php';
            if (is_file($serviceConfigPath)) {
                require_once $serviceConfigPath;
            } else {
                fwrite(STDERR, sprintf("[gateway] optional config missing: %s\n", $serviceConfigPath));
            }
            //此處開始框架其他部件初始化
            ...
            try {
                ...
                \AnserGateway\Worker\GatewayWorker::$router->handle('POST', '/api/orders');
                fwrite(STDOUT, "[route-check] POST /api/orders -> OK\n");
            } catch (\Throwable $e) {
                \AnserGateway\Worker\GatewayWorker::$router = null;
                fwrite(STDERR, sprintf("[route-check] init failed: %s\n", $e->getMessage()));
            }
        ...
        $webWorker->onMessage = static function (TcpConnection $connection, Request $request) use ($config) {
            Coroutine::run(static function () use ($connection, $request, $config): void {
                ...
                if (!\AnserGateway\Worker\GatewayWorker::$router instanceof \AnserGateway\Router\RouterInterface) {
                    $connection->send(new Response(
                        503,
                        ['Content-Type' => 'application/json charset=utf-8'],
                        json_encode([
                            'code' => 503,
                            'msg' => 'gateway router is not initialized',
                            'data' => null,
