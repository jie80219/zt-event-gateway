#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────────
#  SPIFFE E2E Bootstrap — generates join token, registers workloads,
#  and waits for all components to be healthy.
#
#  Runs against the EXTERNAL SPIRE Server (zt-spire-server) via
#  docker exec. The SPIRE Server is owned by a separate project.
#
#  Usage:
#    ./spiffe/e2e/bootstrap.sh
# ──────────────────────────────────────────────────────────────────
set -euo pipefail
...
SPIRE_SERVER_EXEC="docker exec ${SPIRE_SERVER_CONTAINER} /opt/spire/bin/spire-server"
...
# ──────────────────────────────────────────────────────────────────
log "Waiting for SPIRE Server (${SPIRE_SERVER_CONTAINER}) healthcheck..."
elapsed=0
until ${SPIRE_SERVER_EXEC} healthcheck 2>/dev/null; do
    sleep 1
    elapsed=$((elapsed + 1))
...
JOIN_TOKEN=$(${SPIRE_SERVER_EXEC} token generate -spiffeID "spiffe://${TRUST_DOMAIN}/agent" -ttl 600 2>&1 | grep -oP 'Token: \K\S+' || true)

if [ -z "$JOIN_TOKEN" ]; then
    # Fallback: parse differently depending on SPIRE version
    JOIN_TOKEN=$(${SPIRE_SERVER_EXEC} token generate -spiffeID "spiffe://${TRUST_DOMAIN}/agent" -ttl 600 | tail -1 | awk '{print $NF}')
fi
...
# Write token to a local file so the agent can read it (mount as volume)
mkdir -p ./spiffe/shared
echo "${JOIN_TOKEN}" > ./spiffe/shared/join-token
...
elapsed=0
until ${SPIRE_SERVER_EXEC} agent list 2>/dev/null | grep -q "spiffe://${TRUST_DOMAIN}"; do
    sleep 2
    elapsed=$((elapsed + 2))
    if [ "$elapsed" -ge "$MAX_WAIT" ]; then
...
AGENT_ID=$(${SPIRE_SERVER_EXEC} agent list 2>/dev/null | grep -oP 'spiffe://[^\s"]+' | head -1 || echo "")
...
${SPIRE_SERVER_EXEC} entry show 2>&1 || true

# ──────────────────────────────────────────────────────────────────
#  6. Mark bootstrap complete
# ──────────────────────────────────────────────────────────────────
touch ./spiffe/shared/bootstrap-done
